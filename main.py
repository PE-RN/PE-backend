...
@app.post("/token")
async def login(password: Annotated[str, Body()], email: Annotated[EmailStr | None, Body()],
...
async def refresh_token(token: Annotated[str, Body()], controller: Annotated[AuthController, Depends(AuthController.inject_controller)]):
    return await controller.refresh_tokens(token=token)


@app.get("/confirm-email/{temporary_user_id}")
async def confirm_email(temporary_user_id: UUID, controller: Annotated[AuthController, Depends(AuthController.inject_controller)]):
...
async def post_users(user: UserCreate, controller: Annotated[UserController, Depends(UserController.inject_controller)]):
